...
- 日期：2026-08-30
- 关联工单：#53、#54、#55、#56、#57
- 后续状态：resident 容量、挂起容量和跨格式性能验收口径已由 ADR-0041（工单 #60）替代；本记录保留为上一版缓存生命周期与资源边界。
...
标签切换前需要 flush 阅读位置，但每次失活都关闭阅读渲染器会让 EPUB/Markdown/PDF 在 A→B→A 时重新解析内容、重建页面并再次读取托管文件。Readest 的 `BookData` 会保留 BookDocument 与渲染器，这证明保留已解析对象可以改善回切；但 AI Reader 不能照搬其无限驻留的应用层状态，也不能让缓存对象进入持久化工作区。
...
| `suspended` | `ReaderRuntimeCache` + `ReaderRuntime` | 已完成的 EPUB/Markdown/PDF BookDocument、renderer、预算内 DOM/Canvas/范围块；不保留输入接线 | 首次打开完成，缓存键和资源均符合预算 | 精确键命中回到 `active`；预算、指纹/版本/算法变化进入 `evicted` |
...
`EpubBookDocument.attach/detach` 只负责把已打开的 Foliate renderer 移入/移出界面容器；挂起节点暂时放入固定不可见缓存根，防止 Foliate paginator 的未完成帧在 DOM 断开后访问空文档。恢复使用同一对象，不再次调用 `open()`。`PdfBookDocument.attach/detach` 采用同一生命周期，但由 `PdfRenderer` 断开 ResizeObserver、IntersectionObserver、滚动监听和页面调度，清理远离当前位置的 Canvas/解码页并暂停范围请求调度，只保留当前页的最小结果；已登记但尚未派发的 PDF.js 范围队列不丢弃，关闭时才取消，恢复时继续派发，避免 PDF.js 永久等待；detach 最多等待 1 秒让在途读取归零，超时即拒绝缓存并关闭重建。恢复时重新挂载页面窗口并复用原 PDF.js 文档代理。
...
只准入已经完成首次打开的 EPUB、Markdown 和 PDF。未知格式、打开未完成、单项超限和任意无法证明安全一致性的对象都不准入。PDF 挂起时必须收缩到当前页窗口，且不允许继续占用范围读取并发；活动 Runtime 上限继续由最多两个 Editor Group 约束。
...
| 挂起 Runtime | 1 | 1 |
| 挂起 iframe 总数 | 4 | 2 |
| 挂起 Canvas 总数 | 1（PDF 当前页） | 1（PDF 当前页） |
| 挂起解码页总数 | 1（PDF 当前页） | 1（PDF 当前页） |
| 挂起在途范围读取 | 0 | 0 |
| 挂起范围缓存总量 | 16 MiB | 8 MiB |
| 挂起估算资源总量 | 16 MiB | 8 MiB |
...
- `format` 与 `reader-runtime-cache-v2` 状态机版本；PDF 额外使用 `pdfjs-windowed-runtime-v1` 内容算法版本。
...
- 常见 EPUB/Markdown/PDF A→B→A 不再重复解析和创建 BookDocument/底层 renderer；PDF 回切只重新挂载当前页窗口，并在资源预算内保留原 PDF.js 文档。
