...
- **第 31 切片**：跨格式有限活 Reader Runtime 总验收（真实 Chrome 通过六组 EPUB/PDF/Markdown 同格式与跨格式 A→B→A，覆盖双 Editor Group、快速切换、PDF 挂起资源、LRU/关闭/重启和 Markdown 编辑生命周期；600 页以上 PDF 与 Windows Tauri `managed-range` 继续由大型 PDF 门禁覆盖）。对应工单 #57，具体决策见 ADR-0040。
