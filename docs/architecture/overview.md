...
工作台外壳使用 C 原型确认的五套语义配色，默认科技黑且不跟随系统；`WorkbenchAppearance` 在首次 React 绘制前从本机偏好恢复主题与背景光，非法或不可读值回退到科技黑与开启背景光。可重排 EPUB/Markdown 通过 `BookDocument.applyWorkbenchTheme` 使用同一组不透明正文纸张令牌与纯黑/纯白普通正文色；旧 `ReadingTypography.theme` 不覆盖这两种格式。固定版式 EPUB 与 PDF 不接入该正文配色，尚未迁移的格式继续使用其原有阅读排版边界。具体关系见 ADR-0049、ADR-0050。

正文高亮仍以材料级 Annotation 记录保存，但不注册正文点击打开笔记编辑器的行为；材料批注面板区分仅高亮与带文字笔记，笔记编辑从面板进入。

应用顶栏只注册文件、编辑、视图三组真实菜单 Command，分别覆盖导入/备份/恢复/关闭标签、书库筛选/当前材料搜索、书库/目录/阅读排版；编辑器拆分属于当前材料更多操作菜单；不把原型占位动作迁移到生产。
...
按每个 Editor Group 的活动 ReadingView 持有当前 Foliate/PDF View、加载任务、选区和搜索；全应用最多保留两个活动渲染器，并按 ADR-0041/0043 最多保留三个 resident Runtime；按 MaterialId 持有共享的 MarkdownDocumentSession。EPUB/Markdown/PDF 首次打开完成后可按 ReadingView 身份进入 `suspended` Runtime 缓存，挂起时只保留已解析对象和预算内的不可见 renderer/当前 PDF 页，不保留输入接线；桌面可聚合保留两个、平板一个 suspended PDF 当前页，且继续受 16/8 MiB 字节预算约束。缓存命中只重新挂载同一 renderer，PDF 在 ADR-0042 快照条件满足时同步复挂首帧并延迟邻页；无法证明一致或超预算时进入 `evicted`，记录结构化 miss/拒绝并安全重建。Markdown 生命周期和 Workspace State 边界保持不变。
...
工作台外观主题和背景光由 TypeScript 的 `WorkbenchAppearance` 本机偏好管理，不写入 SQLite、Workspace State 或完整书库备份；缺失、非法或不可读时回退到极夜黑与开启背景光。
...
- 全局阅读默认：格式通用的初始阅读偏好。
- 资料级覆盖：该材料所有 View 共享的排版偏好；PDF 只消费适用的正文主题与分页/滚动，页面缩放和适配仍属于 View 级状态。
...
- **第 5 切片**：当前资料目录、规范搜索、导航历史与基本排版（`TocSidebar`、按章节规范 DOM 增量搜索 `canonicalSearch.ts`/`searchStore`/`searchRunner`、文本/安全正则预算与取消、导航历史后退/前进、阅读排版 `typography.ts`、`InterfaceSidebar` 与排版设置持久化）。
...
- **第 32 切片**：三 resident ReadingView 常驻契约（最多三个按 View 计数的 resident Runtime、最多两个 active、桌面/平板 suspended 资源硬预算、真实 EPUB→Markdown→PDF→EPUB A→B→C→A 基线，以及 Readest 文档复用/页面 LRU 取舍记录）。对应工单 #60，具体决策见 ADR-0041。
- **第 33 切片**：PDF ReadingView 回切首帧直接恢复（分页/滚动模式保留当前页 DOM、Canvas、文本层、搜索/批注覆盖与视口快照；相同尺寸/DPR/流向/缩放/适配时跳过当前页重排，首帧后恢复邻页预取；快照不完整或环境变化时安全退化）。对应工单 #61，具体决策见 ADR-0042。
...
- **第 39 切片**：Markdown 复用 EPUB 的五主题配色和精确纯黑兼容，保持共享编辑会话、源码/阅读模式、Recovery Snapshot、双 Editor Group、三 resident Runtime 与跨格式轮换边界；旧三主题只继续服务 PDF/其它未迁移格式。对应工单 #72，具体决策见 ADR-0050；Readest 样式注入行为只作窄范围参考。
...
- **Reader Runtime 压力总验收（Issue #63，继承 #62/#61/#60/#57）**：`verify-reader-runtime-cache.mjs` 在真实 Chrome 中覆盖 EPUB/Markdown/PDF 同格式 A→B→C→A→B→C、混合格式与双组隔离、PDF 多轮首帧、第四项 LRU、单项/累计超预算及安全冷重建；报告 schema 为 `reader-runtime-cache.v6`，记录首次打开、三材料命中、第四项淘汰和冷重建的中位数/P95、结构化 miss/拒绝、关闭一次和资源快照。生命周期还由 Command 测试覆盖关闭、回收站、永久清理、重新关联、版本迁移、Markdown 保存/恢复、整库恢复、退出与重启。600 页以上 PDF 与 Windows `managed-range` 继续由 `verify-reading-performance.mjs` 负责；原生证据按各平台冒烟文档独立采集。
