...
按每个 Editor Group 的活动 ReadingView 持有当前 Foliate/PDF View、加载任务、选区和搜索；全应用最多保留两个活动渲染器，并按 ADR-0041 最多保留三个 resident Runtime；按 MaterialId 持有共享的 MarkdownDocumentSession。EPUB/Markdown/PDF 首次打开完成后可按 ReadingView 身份进入 `suspended` Runtime 缓存，挂起时只保留已解析对象和预算内的不可见 renderer/当前 PDF 页，不保留输入接线；缓存命中只重新挂载同一 renderer，PDF 在 ADR-0042 快照条件满足时同步复挂当前页 DOM/Canvas/文本层/覆盖层并延迟邻页工作，未完成加载、版本/指纹/算法不一致或超出桌面/平板硬预算时进入 `evicted` 并关闭。Markdown 源码模式进入时同样挂起 Foliate，缓冲区、正式保存、放弃或恢复快照变化会先失效材料的全部 Runtime，再按共享会话文本重建非源码视图。Workspace State 仍只保存位置、视口、模式和历史，完整生命周期、三 resident 容量和资源预算见 `docs/adr/0041-three-resident-reader-runtimes.md`；PDF 首帧快照与退化规则见 `docs/adr/0042-pdf-suspended-first-frame-restore.md`；其中 ADR-0040 仍作为被替代的上一版决策记录。
...
- **Reader Runtime 缓存总验收（Issue #62，继承 #61/#60/#57）**：`apps/reader/scripts/verify-reader-runtime-cache.mjs` 在真实 Chrome 中通过真实 `library.openBook`/`reader.activateView` 与 Markdown Command 构造 EPUB、PDF、Markdown，先执行单组和双 Editor Group 的 EPUB→Markdown→PDF→EPUB 三 resident 基线，再覆盖 EPUB↔EPUB、Markdown↔Markdown、PDF↔PDF 及三组跨格式 A→B→A；PDF pair 额外执行 A→B→A→B→A。至少三轮记录每份材料首次可见/首次可交互、切出、缓存命中、BookDocument 身份、来源打开、Foliate renderer、PDF.js 文档/页面取得/光栅化、范围读取、PDF 挂起 Canvas/解码页/在途范围读取、双 Editor Group、源码编辑/保存/恢复快照/重启和 LRU 退化；三 resident 命中不得新增来源、BookDocument、renderer、PDF 页面、光栅化或范围读取。PDF pair 还必须证明回切首帧复用当前页 DOM、Canvas、文本层和高亮层，首帧具备正确页码/滚动位置且首帧前不新增页面取得或光栅化；邻页工作只能在首帧后运行。每轮用同机冷回切中位数/P95 作为动态门槛；报告 schema 为 `reader-runtime-cache.v5`，具体预算与失效规则见 `docs/adr/0041-three-resident-reader-runtimes.md` 与 `docs/adr/0042-pdf-suspended-first-frame-restore.md`。600 页以上 PDF 读取与 Windows `managed-range` 二进制门禁继续由 `verify-reading-performance.mjs` 负责。
