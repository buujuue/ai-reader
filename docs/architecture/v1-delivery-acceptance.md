...
## Issue #63 三材料轮换与资源压力总验收补充

Issue #63 的自动化验收（继承 Issue #62、Issue #61、Issue #60 与 Issue #57）由两条互补命令组成：
...
前一条输出 `reader-runtime-cache.v6`，在真实 Chrome 中通过应用 Command 覆盖：

- EPUB、Markdown、PDF 各自的 A→B→C→A→B→C 三材料逐项命中，以及 EPUB→Markdown→PDF→EPUB 的混合格式单组/双组流程；
- EPUB↔EPUB、Markdown↔Markdown、PDF↔PDF 及 EPUB/PDF/Markdown 三组跨格式 A→B→A；PDF pair 额外执行 A→B→A→B→A；
- 两个 Editor Group 的 ReadingView/位置隔离、快速连续切换、缓存命中无新 BookDocument/renderer/来源/范围读取；PDF 命中在首次可交互前复用当前页 DOM/Canvas/文本层/高亮层，不新增页面取得或光栅化，邻页工作延后；
- 桌面两个 suspended PDF 当前页仍受 16 MiB 聚合预算，平板继续只允许一个；第四项、单项和累计超限记录 LRU、结构化 miss/拒绝、关闭一次与位置冷重建；
