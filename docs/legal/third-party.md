...
- Issue #63 继续检查 Readest 的已解析文档复用、页面结果与局部 LRU；AI Reader 独立实现桌面/平板分档预算、结构化诊断和压力报告，没有直接复制代码，不新增第三方许可义务。
...
- 当前材料搜索:搜索覆盖与渲染相同的规范化章节 DOM，由本项目 `domain/reader/canonicalSearch.ts` 实现稳定文本/Range 映射、索引版本键与正则预算；Foliate 继续提供章节文档读取、CFI 和覆盖层绘制能力，不复制其应用层搜索栏。增量进度、取消与命中跳转的结果编排模式参考 Readest `apps/readest-app/src/app/reader/components/sidebar/SearchBar.tsx`,但本项目为独立的最小实现(见 `workbench/searchRunner.ts` 与 `components/SearchBar.tsx`),不复制其搜索缓存、历史或应用层外围逻辑。
...
- 可重排 EPUB 全局主题(工单 #71):行为参考 Readest `apps/readest-app/src/utils/style.ts` 的 `getColorStyles`/纯黑兼容选择器与 `apps/readest-app/src/app/reader/components/FoliateViewer.tsx` 的章节样式刷新。AI Reader 独立实现 `domain/reader/epubTheme.ts` 的五主题固定令牌、可逆 CSS 变量和精确纯黑声明转换，不复制 Readest 应用层代码，也不引入其链接重着色、局部背景修复、图片滤镜或 PDF 全局主题；Readest 项目以 AGPL-3.0 发布，来源仓库为 [readest/readest](https://github.com/readest/readest)。
- Markdown 全局主题(工单 #72):继续参考上述 Readest 样式注入与章节刷新行为，但仅复用 AI Reader 已有的 `domain/reader/epubTheme.ts`、`typography.ts` 与 `foliateViewHost.ts` 窄能力；未直接移植 Readest 代码，不新增第三方许可义务，不引入其链接重着色、局部背景修复、图片滤镜或 PDF 全局主题。
- PDF 阅读内核:范围读取并发上限、过期渲染取消、画布内存预算与滚动窗口化思路,参考 Readest 的 `packages/foliate-js/pdf.js` 与 `packages/foliate-js/src/foliate/view/pdf.js`。PDF 首页来源封面的一次性 `renderPage` 生命周期也参考 `packages/foliate-js/pdf.js` 的首页渲染路径；本项目为独立的最小实现(`domain/reader/pdf/` 的 `pdfRangeTransport.ts`、`pdfCover.ts`、`pdfPageRenderer.ts`、`pdfRenderer.ts`),不复制其应用层 PDF 工具栏、页面缩略图、双页/连续滚动等外围逻辑。
- PDF 滚动占位调度（工单 #45）:行为参考 Readest `packages/foliate-js/fixed-layout.js` 的 `#initScrollMode`、`#scheduleScrollPages`、`#loadScrollPage`、`#teardownScrollPage`、`captureScrollModeAnchor` 与 `restoreScrollModeAnchor`（来源仓库 [readest/readest](https://github.com/readest/readest)，AGPL-3.0）。AI Reader 独立重写为 `PdfRenderer` 的占位布局、`IntersectionObserver` 调度、页内比例锚点和 PDF.js 页面生命周期，没有直接复制 Readest 代码；保留该来源名称、仓库链接与 AGPL-3.0 署名说明。
...
- 托管材料范围读取(工单 #33):分块读取、LRU 访问顺序和同一分块并发去重模式参考 Readest `apps/readest-app/src/utils/file.ts` 的 `NativeFile`/`RemoteFile`。Readest 项目以 AGPL-3.0 发布，来源仓库为 [readest/readest](https://github.com/readest/readest)；AI Reader 只独立重写底层缓存模式，保留本项目的稳定 MaterialId、typed Repository、8 MiB 半开范围限制和 Rust 私有路径边界，不移植其路径授权或应用层代码。
- Windows PDF 二进制范围协议(工单 #44):WebView 网络栈承载本地二进制范围响应的模式参考 Readest `apps/readest-app/src/utils/file.ts` 的 `RemoteFile` 与 `apps/readest-app/src-tauri/src/range_file.rs` 的 `rangefile` custom URI handler。Readest 项目以 AGPL-3.0 发布，来源仓库为 [readest/readest](https://github.com/readest/readest)；AI Reader 独立重写为 `MaterialId + offset + length` 查询，并复用 Rust 的活跃 ready 材料校验，不复制 Readest 的绝对路径 URL、asset scope 或前端路径授权模型。
- Android WebView 就绪探测(工单 #64):行为参考 Readest `apps/readest-app/docs/testing.md` 与 `apps/readest-app/src/__tests__/android/helpers/cdp.ts` 的 `webview_devtools_remote_<pid>` 转发、CDP 页面探测和“poll, don't sleep”约定。AI Reader 独立实现 `scripts/android-webview-probe.mjs`，增加自身的工作台 DOM、阶段证据和 PNG 像素校验，不直接复制 Readest 代码；Readest 项目以 AGPL-3.0 发布，来源仓库为 [readest/readest](https://github.com/readest/readest)。
- 完整书库备份与树结构恢复(工单 #52):行为层面参考 Readest `apps/readest-app/src/services/backupService.ts` 的“备份前清理设备态、恢复时校验结构与逐项失败隔离”思路；Readest 项目以 AGPL-3.0 发布，AI Reader 未直接复制该应用层代码，而是独立采用 Rust SQLite Online Backup API、版本化 tar manifest、文件夹关系校验和可恢复整库切换。该实现记录在 `apps/reader/src-tauri/src/db/backup.rs` 与 ADR-0037。
