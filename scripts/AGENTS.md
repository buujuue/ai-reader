...
- `android-webview-probe.mjs`：Android 原生冒烟的真实 WebView/CDP 有界就绪探测器；按目标进程、前台 Activity、WebView DevTools 页面和可识别工作台 DOM 判定启动、触摸、重启三个阶段是否就绪，并拒绝系统错误对话框或不属于目标包的 UIAutomator 证据。配套回归：`pnpm test:android-smoke`。
