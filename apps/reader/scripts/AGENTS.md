...
| `verify-reader-runtime-cache.mjs` | 工单 #53/#54 Reader Runtime 缓存基线:真实 Chrome 通过 `library.openBook`/`reader.activateView` Command 构造 EPUB/Markdown A→B→A，记录缓存命中、切换、对象创建、范围读取、资源和可获得堆内存；门槛来自同机冷回切中位数/P95。 | `pnpm test:reader-runtime-cache`；详见 `docs/adr/0040-bounded-reader-runtime-cache.md` |
