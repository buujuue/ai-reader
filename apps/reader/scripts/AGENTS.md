...
| `verify-reader-runtime-cache.mjs` | 工单 #62 Reader Runtime 缓存总验收（继承 #61/#60/#57）:真实 Chrome 通过 `library.openBook`/`reader.activateView` Command 验证单组与双组 EPUB→Markdown→PDF→EPUB 三 resident、同格式/跨格式 A→B→A、按 ReadingView 计数的 LRU 淘汰、PDF 首帧复用、Markdown 编辑/保存/恢复快照/重启和退出清理；记录首次可见/可交互、缓存命中、BookDocument/renderer/PDF.js 页面与光栅化、范围读取及挂起资源。 | `pnpm test:reader-runtime-cache`；详见 `docs/adr/0041-three-resident-reader-runtimes.md` |
