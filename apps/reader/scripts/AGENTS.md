...
| `verify-reader-runtime-cache.mjs` | 工单 #57 Reader Runtime 缓存总验收:真实 Chrome 通过 `library.openBook`/`reader.activateView` Command 验证 EPUB/PDF/Markdown 同格式与跨格式 A→B→A，记录缓存命中、位置、对象创建、范围读取、PDF 挂起资源、双 Editor Group、Markdown 编辑/保存/恢复快照/重启和 LRU 退化；PDF 使用实际 PDF.js 与托管范围来源。 | `pnpm test:reader-runtime-cache`；详见 `docs/adr/0040-bounded-reader-runtime-cache.md` |
