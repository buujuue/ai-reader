...
- `bootstrap.ts`：组装 `AppServices`（`CommandRegistry` + `WorkspaceRepository` + `ImportRepository` + `BackupRepository` + `FilePicker` + `BackupDestinationPicker` + `ExternalUrlOpener` + `WindowLifecycle`）。`isTauriRuntime()` 检测 `__TAURI_INTERNALS__`，据此选择 Tauri Adapter 或内存 Adapter；桌面端的 `WindowLifecycle` 只暴露关闭请求监听与销毁窗口两项能力；`createAppServices()` 注册工作台、书库、备份与阅读命令，内存降级时用演示 EPUB 种子化书库，可注入平台窄接口供测试。
