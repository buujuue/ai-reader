...
- `AnnotationSidebar.tsx`：导出 `AnnotationPanel`，按 materialId 显示材料级运行时批注覆盖面板，支持引文/笔记筛选、失联标识，以及通过 `annotation.goTo` 跳转正文或 PDF 区域；查看、编辑、关闭和导出均通过 Command/运行时外壳边界完成，面板不进入 Workspace State。
