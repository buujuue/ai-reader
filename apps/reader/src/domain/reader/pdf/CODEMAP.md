...
- `pdfRenderer.ts`：布局管理器。分页/滚动两种流向、缩放与页面适配（宽度/高度/整页/实际大小）；分页模式在当前页完成后取得并预渲染下一页，翻页复用脱离 DOM 的结果，只保留当前页和下一页的渲染器；滚动模式使用视口窗口化，并无论页面高度都额外渲染当前页的下一页，窗口移动时复用仍在窗口内的结果，之后释放离开窗口的画布；同时负责画布内存预算、解码页 LRU 释放，并暴露按页设置高亮的方法。
