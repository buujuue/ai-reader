...
- `epubTheme.ts`：可重排 EPUB 复用的五套全局正文纸张/前景令牌，以及精确纯黑 CSS/XHTML 声明转换；只生成可逆 CSS 变量，不改写托管材料、SVG、显式彩色或局部背景。
