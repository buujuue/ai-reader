...
- `epubInspector.ts`：`inspectEpub` 在 `stage → inspect → commit` 的 inspect 阶段验证 container.xml、OPF、manifest、spine、首个可读章节、DRM/字体混淆与硬资源预算；元数据和封面可观察结果由 foliate-js 产生，检查器只负责安全预算与报告，整书失败抛稳定 `EpubInspectError`。
