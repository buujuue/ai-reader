...
领域层定义可序列化的领域模型与 typed Repository 接口，是前端与 Rust 平台核心之间的契约边界。当前含 `workspace/`（工作区）与 `library/`（书库与导入）两个子域。
...
domain/
├── app/
│   ├── bootstrap.ts      创建 Workspace/Import Repository 与 FilePicker
