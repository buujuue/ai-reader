...
- `readerRuntimeCache.ts`：Reader Runtime 有界缓存深模块，生成包含 View/Material/指纹/文档版本/算法版本的缓存键，执行 EPUB/Markdown/PDF 准入、桌面/平板资源硬预算与 LRU 淘汰，并为真实浏览器基线提供命中/失效诊断；PDF 挂起资源限制为当前页 Canvas/解码页和零在途范围读取。缓存不直接关闭对象，关闭由 `readerCommands.ts` 的 Runtime 所有者统一执行。
...
- `readerCommands.ts`：阅读 Command 唯一实现入口，负责打开/激活/翻页/跳转/搜索/排版/关闭/恢复；`reader.activateView` 先 flush/清理位置、输入、搜索和焦点，再按 ADR-0040 挂起旧标签 Runtime，命中 EPUB/Markdown/PDF 缓存时以已 flush 的 Workspace 位置校验并 attach 原 renderer，失效/超预算时安全重建；同时向 Markdown Command 提供源码模式 Runtime 的挂起/恢复 seam，并让材料刷新跳过源码模式视图。`mountViewDocument` 在新 Runtime 调用 open、缓存命中时 attach，并接线位置持久化与导航事件。EPUB/Markdown 的托管 Source、Foliate 语义和 PDF.js 读取均保持既有窄接口边界。`scripts/verify-reader-runtime-cache.mjs` 通过同一真实应用 Seam 执行 Issue #57 的六组格式 pair 与两轮 A→B→A→B→A 总验收。
...
`workbench/` 构建在 `commands/` 与 `domain/` 之上，向下游（`app/`、`components/`）提供 Store 与命令实现；它不反向依赖 UI 组件。可序列化状态（标签、阅读位置）在 Workspace Store，活对象（BookDocument）在 Reader Runtime，有限挂起对象由 Reader Runtime Cache 管理，两者严格分离；批注导出只读取材料级 AnnotationRepository 并经 AnnotationExportWriter 调用 Rust，不读取本地文件或数据库。
## 完整书库备份恢复
...
- EPUB 打开路径获取一次 `ManagedFileSource`，检查器、BookDocument 与 Foliate 惰性 ZIP loader 共享该来源；同条目并发读取复用进行中的 Promise，除进入 ADR-0040 的挂起 Runtime 外不保留正文派生对象。PDF 打开命令只构造文档，首屏挂载才创建 PDF.js 文档；每个新建活动 Runtime 各自只创建一次。
