...
- `readerRuntimeCache.ts`：Reader Runtime 有界缓存深模块，生成包含 View/Material/指纹/文档版本/算法版本的缓存键，执行 EPUB/Markdown 准入、桌面/平板资源硬预算与 LRU 淘汰，并为真实浏览器基线提供命中/失效诊断；PDF 明确拒绝。缓存不直接关闭对象，关闭由 `readerCommands.ts` 的 Runtime 所有者统一执行。
...
- `readerCommands.ts`：阅读 Command 唯一实现入口，负责打开/激活/翻页/跳转/搜索/排版/关闭/恢复；`reader.activateView` 先 flush/清理位置、输入、搜索和焦点，再按 ADR-0040 挂起旧标签 Runtime，命中 EPUB/Markdown 缓存时 attach 原 renderer，PDF/失效/超预算时安全重建；同时向 Markdown Command 提供源码模式 Runtime 的挂起/恢复 seam，并让材料刷新跳过源码模式视图。`mountViewDocument` 在新 Runtime 调用 open、缓存命中时 attach，并接线位置持久化与导航事件。EPUB/Markdown 的托管 Source、Foliate 语义和 PDF.js 读取均保持既有窄接口边界。
