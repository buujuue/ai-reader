...
- `readerRuntimeCache.ts`：Reader Runtime 有界缓存深模块，生成包含 View/Material/指纹/文档版本/算法版本的缓存键，执行三 resident、最多两个 active、最多两个 suspended 的 EPUB/Markdown/PDF 准入、桌面/平板资源硬预算与 LRU 淘汰，并为真实浏览器基线提供命中/失效诊断；PDF 挂起资源限制为当前页 Canvas/解码页和零在途范围读取。缓存不直接关闭对象，关闭由 `readerCommands.ts` 的 Runtime 所有者统一执行。
...
- `readerCommands.ts`：阅读 Command 唯一实现入口，负责打开/激活/翻页/跳转/搜索/排版/关闭/恢复；`reader.activateView` 先 flush/清理位置、输入、搜索和焦点，再按 ADR-0041 挂起旧标签 Runtime，命中 EPUB/Markdown/PDF 缓存时以已 flush 的 Workspace 位置校验并 attach 原 renderer，PDF 兼容 ADR-0042 快照时在 `mountViewDocument` 跳过重复位置导航，resident 超限按 LRU 淘汰 suspended，失效/超预算时安全重建；同时向 Markdown Command 提供源码模式 Runtime 的挂起/恢复 seam，并让材料刷新跳过源码模式视图。`mountViewDocument` 在新 Runtime 调用 open、缓存命中时 attach，并接线位置持久化与导航事件。EPUB/Markdown 的托管 Source、Foliate 语义和 PDF.js 读取均保持既有窄接口边界。`scripts/verify-reader-runtime-cache.mjs` 通过同一真实应用 Seam 执行 Issue #62 的单组/双组三 resident 回切、Issue #61 的 PDF 首帧回切及 Issue #57 的格式 pair 回归。
