...
- `readerCommands.ts`：阅读 Command 唯一实现入口，负责打开/激活/翻页/跳转/搜索/排版/关闭/恢复；`reader.activateView` 先 flush/清理位置、输入、搜索和焦点，再按 ADR-0040 挂起旧标签 Runtime，命中 EPUB/Markdown 缓存时 attach 原 renderer，PDF/失效/超预算时安全重建；`mountViewDocument` 在新 Runtime 调用 open、缓存命中时 attach，并接线位置持久化与导航事件。EPUB/Markdown 的托管 Source、Foliate 语义和 PDF.js 读取均保持既有窄接口边界。
