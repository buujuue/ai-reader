...
- `workbenchCommands.ts`：工作台 Command 的唯一实现入口。`registerWorkbenchCommands` 注册书库/目录/界面侧栏切换、工作台主题与背景光本机偏好、筛选聚焦、文件夹/未归类展开状态、主要材料设置、材料批注覆盖面板、笔记编辑入口、阅读排版入口、`app.back`、`shell.dismissDialog`、`workbench.focusEditorGroup` 与 `workbench.saveState`；三类活动面板的持久化状态始终互斥，外观偏好、搜索投影和临时展开、覆盖面板等不写入 Workspace State。
