...
| `domain/` | 领域层：`workspace/` 工作区（Editor Group、阅读视图、阅读位置、导航历史）、`library/` 书库与导入（含内建最小 `EpubInspector`、`materialFormat` 格式推断、`libraryFilter` 即时筛选）、`annotation/` 批注（`Annotation`/`TextAnchor` 类型、`textAnchor.ts` 文本锚点构建与恢复、`AnnotationRepository` 及内存/localStorage/Tauri 三个 Adapter 与共享契约）、`reader/` 阅读文档（`BookDocument`、`EpubBookDocument`、`ReadingLocation`、`NavigationHistory`、`TocItem`、Foliate 视图宿主、内容清洗器）、`tauriInvoke.ts` 共享 invoke 类型。 |
