...
| `workbench/` | `workspaceStore.ts`（可序列化工作区状态 + 导航历史动作）、`readerRuntime.ts`（活 BookDocument）、`positionPersister.ts`（阅读位置节流/flush）、`readerCommands.ts`（打开/翻页/目录跳转/后退前进/外部链接/关闭/恢复命令 + 历史接线）、`workbenchCommands.ts` 命令注册、`libraryStore.ts`（书库）、`libraryCommands.ts`（导入/刷新/元数据覆盖命令）、`importBook.ts`（stage→inspect→commit 编排）、`shellUiStore.ts` 外壳 UI 状态（含元数据编辑器、外部链接、目录开关），及对应测试。 |
