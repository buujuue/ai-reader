...
- Workspace State 必须可序列化；Reader Runtime 活对象（视图、选区、加载任务）不得进入持久化状态。每个 Editor Group 仅保留活动阅读器，全应用最多两个活动渲染器；EPUB/Markdown/PDF 完成打开后可由 `readerRuntimeCache.ts` 按有界预算挂起，切换由 `readerCommands.ts` 先 flush 并清除输入/搜索接线，PDF 只保留 PDF.js 文档和当前页的预算内结果；材料失效与整库恢复必须释放旧对象。Issue #57 的总验收由 `scripts/verify-reader-runtime-cache.mjs` 统一覆盖，浏览器设备模拟不计作原生证据。
