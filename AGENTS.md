...
- Workspace State 必须可序列化；Foliate/PDF View、加载任务、当前选区等 Reader Runtime 活对象不得混入持久化状态。每组只保留一个活动渲染器，全应用最多两个 active、全应用最多三个 resident Runtime；EPUB/Markdown/PDF 已完成 Runtime 可按 ADR-0041/0043 进入有界 suspended 缓存，resident 超限只按 LRU 淘汰 suspended，PDF 挂起只保留 PDF.js 文档和当前页的预算内结果；桌面最多聚合保留两个、平板最多一个 suspended PDF 当前页，且仍受字节预算约束。
