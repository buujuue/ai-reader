...
- 书库主侧栏默认使用真实、可持久化的书库文件夹树；文件夹拥有稳定 ID 和显式父级，最多五层，同一父级名称不区分大小写唯一，名称自动去除首尾空格并拒绝空值、路径分隔符、控制字符和超过 80 个字符。新建支持顶层/子文件夹，创建后立即命名，Enter 保存、Escape 取消；改名保持父级不变，不提供移动文件夹。当前阅读材料先统一显示在树底部“未归类”，并继续保留导入、标题/作者筛选、打开、元数据编辑、回收站和封面网格能力；材料归类另作为后续切片。
