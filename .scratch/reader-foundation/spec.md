...
- 书库主侧栏默认使用真实、可持久化的书库文件夹树；文件夹拥有稳定 ID 和显式父级，最多五层，同一父级名称不区分大小写唯一，名称自动去除首尾空格并拒绝空值、路径分隔符、控制字符和超过 80 个字符。新建支持顶层/子文件夹，创建后立即命名，Enter 保存、Escape 取消；改名保持父级不变，不提供移动文件夹。阅读材料作为所属文件夹的叶子节点，未归类材料固定在树底部；新导入材料默认未归类，单本材料可经“移动到……”菜单归入任意已有文件夹或移回未归类。材料按有效标题稳定排序，未归类区域有书时默认展开且允许折叠，没有书时只保留轻量标题；筛选同时匹配文件夹名称、标题和作者并保留命中路径。继续保留导入、打开、元数据编辑、回收站和封面网格能力。
