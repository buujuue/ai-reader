...
- 材料更多菜单位于阅读工具栏右侧，提供查看/导出该材料批注、设置主要阅读材料、编辑元数据和移入回收站；阅读排版统一由“界面”活动面板调整，工具栏按钮和“视图 → 阅读排版”只作为聚焦该面板的快捷入口，不创建独立对话框。
- 工作台外壳采用 C 原型确认的科技黑、简洁白、护眼橙、清新绿和甜蜜粉五套主题，背景光作为独立本机偏好保存；可重排 EPUB 的正文基础纸张色与普通正文色跟随同一全局主题，旧阅读主题值不再覆盖适用 EPUB。固定版式 EPUB 与 PDF 保持原有正文配色，尚未迁移的其它格式继续沿用各自边界。
- 应用顶栏只提供文件、编辑、视图三组真实菜单：导入/备份/恢复/关闭标签、书库筛选/当前材料搜索、书库/目录/阅读排版；编辑器拆分属于当前材料更多操作菜单；所有菜单项执行稳定 Command，不保留原型占位动作。
